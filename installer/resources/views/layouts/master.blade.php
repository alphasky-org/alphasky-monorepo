...
<head>
    ...
    <meta
        name="description"
        content="Installation Alphasky CMS v{{ get_cms_version() }}"
    >
        ...
        @if (Session::has('success_msg') || Session::has('error_msg') || (isset($errors) && $errors->any()) || isset($error_msg))
            <script type="text/javascript">
                $(function() {
                    ...
                    Alphasky.showSuccess('{{ session('success_msg') }}');
                    @endif
                    @if (Session::has('error_msg'))
                    Alphasky.showError('{{ session('error_msg') }}');
                    @endif
                    @if (isset($error_msg))
                    Alphasky.showError('{{ $error_msg }}');
                    @endif
                    @if (isset($errors))
                    @foreach ($errors->all() as $error)
                    Alphasky.showError('{{ $error }}');
                    @endforeach
                    @endif
                })
            </script>
