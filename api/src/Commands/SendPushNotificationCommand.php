...
namespace Alphasky\Api\Commands;

use Alphasky\Api\Models\PushNotification;
use Alphasky\Api\Services\PushNotificationService;
...
class SendPushNotificationCommand extends Command
{
    protected $signature = 'cms:push-notification:send
                            ...
                            {--target-value= : Target value (android/ios for platform, member/admin for user_type, user_id for user)}
                            ...
                            {--user-type=member : User type when target is user (member, admin)}
    ...
    protected function handleInteractive(): int
    {
        ...
        if ($target === 'platform') {
            ...
            $targetValue = $this->choice('Select user type', ['member', 'admin']);
        } elseif ($target === 'user') {
            $userType = $this->choice('Select user type', ['member', 'admin']);
    ...
    protected function sendBasedOnTarget(array $notificationData): array
    {
        ...
        switch ($target) {
            ...
            case 'user':
                $userType = $notificationData['user_type'] ?? 'member';
