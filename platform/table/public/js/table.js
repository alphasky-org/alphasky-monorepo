;(($, DataTable) => {
    ...
    DataTable.ext.buttons.excel = {
        ...
        text: (dt) => {
            ...
            ${dt.i18n(
                'buttons.excel',
                AlphaskyVariables.languages.tables.excel ? AlphaskyVariables.languages.tables.excel : 'Excel'
            )}`
        },
    ...
    DataTable.ext.buttons.export = {
        ...
        text: (dt) => {
            ...
            ${dt.i18n(
                'buttons.export',
                AlphaskyVariables.languages.tables.export ? AlphaskyVariables.languages.tables.export : 'Export'
            )}`
        },
    ...
    DataTable.ext.buttons.csv = {
        ...
        text: (dt) => {
            ...
            ${dt.i18n(
                'buttons.csv',
                AlphaskyVariables.languages.tables.csv ? AlphaskyVariables.languages.tables.csv : 'CSV'
            )}`
        },
    ...
    DataTable.ext.buttons.print = {
        ...
        text: (dt) => {
            return `<svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                ...
                ${dt.i18n(
                    'buttons.print',
                    AlphaskyVariables.languages.tables.print ? AlphaskyVariables.languages.tables.print : 'Print'
                )}`
        },
    ...
    DataTable.ext.buttons.reset = {
        ...
        text: (dt) => {
            return `<svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                ...
                ${dt.i18n(
                    'buttons.reset',
                    AlphaskyVariables.languages.tables.reset ? AlphaskyVariables.languages.tables.reset : 'Reset'
                )}`
        },
    ...
    DataTable.ext.buttons.reload = {
        ...
        text: (dt) => {
            return `
                ...
                ${dt.i18n(
                    'buttons.reload',
                    AlphaskyVariables.languages.tables.reload ? AlphaskyVariables.languages.tables.reload : 'Reload'
                )}`
        },
...
// To reload data for a specific table instance, use:
// $('#table').DataTable().ajax.reload(function() {
//     console.log('Data reloaded successfully!');
//     // Example: Show a success message, update a counter, etc.
// });
    ...
    class TableManagement {
        ...
        init() {
            ...
            if ($.fn.dataTable) {
                ...
                $(document).on('error.dt', (e, settings, techNote, message) => {
                    Alphasky.handleDatatableError({ responseJSON: { message } });
                });
 
            }
           
            $(document).on('click','td.editable', (event) => {
              
                
                   
                console.log('Initializing editable table cells...');
                $(document).find('td.editable').each((index, el) => {                  
        ...
        makeEditable($cell) {
            ...
            $cell.on('focus', function() {
                $(this).addClass('table-editing');             
            });

            $cell.on('blur keyup change', function(e) {
                
                $(this).removeClass('table-editing');
                ...
                if (e.type === 'blur' || e.key === 'Enter') {
                    ...
                    if (newValue !== oldValue) {
                        ...
                        // Trigger custom event on value change
                        $this.trigger('valueChanged', [newValue, oldValue]);
                    }
                }
            });
            ...
            $cell.on('valueChanged', (event, newValue) => {
                const $this = $(event.currentTarget);
                const table = $this.closest('table').DataTable();
                const row = table.row($this.closest('tr'));
                const rowData = row.data();
                const columnName = table.column($this).dataSrc();
                const pk = rowData.id;
                const url = '/admin/tables/inline-edit';
                const model = table.init().model;
                
                $httpClient.make()
                    .get(url, {
                    name: columnName,
                    value: newValue,
                    model: $('input[name="ClassName"]').val(),
                    pk: pk,
                    })
                    .then(({ data }) => {
                        if (data.error) {
                            Alphasky.showError(data.message);
        ...
        handleActionsRow() {
            ...
            $(document).on('click', '.delete-many-entry-trigger', (event) => {
                ...
                if (ids.length === 0) {
                    Alphasky.showError(
                        AlphaskyVariables.languages.tables.please_select_record
                            ? AlphaskyVariables.languages.tables.please_select_record
