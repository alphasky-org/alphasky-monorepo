...
class Alphasky {
    ...
    constructor() {
        ...
        Alphasky.initNavbarMinimal()
        Alphasky.initResources()
        Alphasky.initGlobalResources()
        Alphasky.handleCounterUp()
        Alphasky.initMediaIntegrate()

        if (
            AlphaskyVariables &&
            AlphaskyVariables.authorized === '0' &&
            typeof AlphaskyVariables.authorize_url !== 'undefined'
        ) {
    ...
    static blockUI(options) {
        ...
        Alphasky.showLoading(options.target)
    }

    static unblockUI(target) {
        Alphasky.hideLoading(target)
    }
    ...
    static showNotice(messageType, message, messageHeader = '') {
        ...
        if (Alphasky.noticesTimeout[key]) {
            clearTimeout(Alphasky.noticesTimeout[key])
        }

        Alphasky.noticesTimeout[key] = setTimeout(() => {
            if (!messageHeader) {
                switch (messageType) {
                    case 'error':
                        messageHeader = AlphaskyVariables.languages.notices_msg.error
                        break
                    case 'success':
                        messageHeader = AlphaskyVariables.languages.notices_msg.success
                        break
                }
            }
    ...
    static handleError(data) {
        if (typeof data.errors !== 'undefined' && !_.isArray(data.errors)) {
            Alphasky.handleValidationError(data.errors)
        } else {
            if (typeof data.responseJSON !== 'undefined') {
                if (typeof data.responseJSON.errors !== 'undefined') {
                    if (data.status === 422) {
                        Alphasky.handleValidationError(data.responseJSON.errors)
                    }
                } else if (typeof data.responseJSON.message !== 'undefined') {
                    Alphasky.showError(data.responseJSON.message)
                } else {
                    $.each(data.responseJSON, (index, el) => {
                        $.each(el, (key, item) => {
                            Alphasky.showError(item)
                        })
                    })
                }
            } else {
                Alphasky.showError(data.statusText)
            }
        }
    }

    static handleDatatableError(error) {
        let errorMessage = AlphaskyVariables.languages.tables.error_loading
            ? AlphaskyVariables.languages.tables.error_loading
        ...
        Alphasky.showError(errorMessage);
    }
    ...
    static async copyToClipboard(textToCopy, parentTarget) {
        if (navigator.clipboard && window.isSecureContext) {
            ...
            Alphasky.unsecuredCopyToClipboard(textToCopy, parentTarget)
        }
    }
    ...
    static initResources() {
        ...
        $.each($(document).find('.select-search-ajax'), function (index, element) {
            ...
            if ($element.data('url')) {
                ...
                Alphasky.select(element, options)
        ...
        Alphasky.initDatePicker('.datepicker')

        if (jQuery().textareaAutoSize) {
            $('textarea.textarea-auto-height').textareaAutoSize()
        }

        Alphasky.initCodeEditorComponent()
        Alphasky.initColorPicker()
    ...
    static initGlobalResources() {
        ...
        Alphasky.initFieldCollapse()
        Alphasky.initTreeCheckboxes()
        Alphasky.initClipboard()
        Alphasky.initDropdownCheckboxes()
    }
    ...
    handleTurnOffDebugMode() {
        ...
        $submit.on('click', function (event) {
            ...
            Alphasky.showButtonLoading($submit[0])
            ...
            $httpClient
                ...
                .finally(() => {
                    Alphasky.hideButtonLoading($submit[0])
                })
        })
    }
    ...
    static initMediaIntegrate() {
        if (jQuery().rvMedia) {
            Alphasky.gallerySelectImageTemplate = `
            ...
            const gallerySelectImages = function (files, $currentBoxList, excludeIndexes = []) {
                let template = Alphasky.gallerySelectImageTemplate
    ...
    static initCodeEditor(id, type = 'css') {
        ...
        $('.handle-tool-drag').mousedown((event) => {
            ...
            $('body').attr('data-dragtool', _self.attr('id')).on('mousemove', Alphasky.onDragTool)
            $(window).on('mouseup', Alphasky.onReleaseTool)
        })
    }
    ...
    static onReleaseTool() {
        $('body').off('mousemove', Alphasky.onDragTool)
        $(window).off('mouseup', Alphasky.onReleaseTool)
    }
    ...
    processAuthorize() {
        ...
        $httpClient
            .makeWithoutErrorHandler()
            .post(AlphaskyVariables.authorize_url)
            .then(() => {
    ...
    countMenuItemNotifications() {
        ...
        if ($menuItems.length) {
            ...
            $httpClient
                .make()
                .get($menuItems.data('url') || AlphaskyVariables.menu_item_count_url)
                .then(({ data }) => {
        ...
        $(document).on('click change', '[data-bb-toggle="collapse"]', (e) => {
            ...
            switch (type) {
                ...
                case 'button':
                    ...
                    break
                default:
                    console.warn(`[Alphasky] Unknown type ${type} of collapse`)
            }
        })
    ...
    static initCodeEditorComponent() {
        $(document)
            ...
            .each(function () {
                ...
                Alphasky.initCodeEditor(this, this.dataset.mode || 'htmlmixed')
            })
    }
    ...
    static initLightbox() {
        ...
        a.forEach((element) => {
            ...
            if (!instance[instanceName]) {
                instance[instanceName] = Alphasky.lightbox()
            }
    ...
    static initClipboard() {
        $(document).on('click', '[data-bb-toggle="clipboard"]', async (e) => {
            ...
            await Alphasky.copyToClipboard(text, clipboardParentTarget)

            if (copiedMessage) {
                Alphasky.showSuccess(copiedMessage)
            }
...
function updateTablesDisplay() {
    ...
    // Iterate over each table element found
    tables.forEach(table => {
        // Check if the table has a parent element
         table.style.display = 'table';
        const parent = table.parentElement;

        if (parent) {
            // Get the actual computed width of the table and its parent in pixels
            // offsetWidth returns the width of the element, including padding and border
            const tableWidth = table.offsetWidth;
            const parentWidth = parent.offsetWidth+50;

            // Check if the table width is NOT equal to the parent width
            // We use a small tolerance (e.g., 1px) due to potential floating point inaccuracies 
            // in browser rendering, although strict comparison (tableWidth !== parentWidth) might suffice 
            // in many cases. Here we assume "expanded to 100%" means exactly equal width.
            
            // If the table is NOT fully expanded (width is not 100% of the parent)
            if (tableWidth < parentWidth) {
                // Not 100% expanded -> use table layout properties
                table.style.display = 'table';
            } else {
                // Fully expanded (100% of the parent width or more) -> use block layout
                // We assume 'block' is desired when it is fully expanded/responsive
...
// 2. Run the function on window resize to handle dynamic changes
