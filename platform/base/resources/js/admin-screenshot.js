 ...
 * Can be used with any Alphasky CMS project.
 *
 * Usage:
 ...
 *     ],
 *     "portal": [
 *       { "name": "portal-login", "path": "/member/login", "noAuth": true }
 *     ]
 *   }
 */
...
Options:
  ...
  --memberEmail=<str>    Member email for portal auto-login
  --memberPassword=<str> Member password for portal auto-login
  --portal           Also capture member portal pages from config
...
async function memberLogin(page, site, options) {
    if (!options.memberEmail || !options.memberPassword) {
        return false;
    }

    console.log('\nLogging into member portal...');
    const loginUrl = `${site}/member/login`;
    ...
    try {
        ...
        if (emailField && passwordField) {
            await emailField.type(options.memberEmail);
            await passwordField.type(options.memberPassword);
        ...
        console.log('Member auto-login failed: ' + e.message);
    }
    return false;
}
...
async function main() {
    ...
    // Capture member portal pages
    if (options.portal && config.portal && config.portal.length > 0) {
        console.log('');
        console.log('Capturing Member Portal Screenshots:');
        console.log('-'.repeat(40));
        ...
        if (authPages.length > 0) {
            const memberLoggedIn = await memberLogin(page, options.site, options);
            if (!memberLoggedIn && !options.headless) {
                console.log('\nPlease login to member portal for authenticated pages...');
                await page.goto(`${options.site}/member/login`, { waitUntil: 'networkidle2' });
                await prompt('Press Enter after you have logged in to member portal...');
            }
