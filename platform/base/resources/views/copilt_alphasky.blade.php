...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        async function sendMessage() {
            ...
            const menuKey = document.getElementById('ai-copilot-menu').value;
            ...
            // أضف رسالة بوت مبدئية ليتم تحديثها لاحقاً
            let lastBotMsg = document.createElement('div');
            lastBotMsg.classList.add('chat-message', 'bot-message');
            lastBotMsg.textContent = 'جاري التنفيذ...';
            ...
            try {
                ...
                const eventSource = new EventSource('{{ route('system.alphasky') }}?' + new URLSearchParams({
                        userInput: text
                        , key: menuKey
                    }));
                ...
                eventSource.onmessage = function(event) {
                    ...
                    // Display in UI
                    lastBotMsg.innerHTML += `   <div class="chat-message bot-message">${data.message}</div>`;

                    // Close connection if task is finished
                    if (data.message === 'إنهاء المهمة') {
                        eventSource.close();
                    }
                };
                ...
                lastBotMsg.textContent = 'حدث خطأ أثناء الاتصال بالخادم.';
            }
            ui.input.value = '';
            attachedFiles = [];
...
<div id="ai-copilot-panel">
    ...
    <div id="ai-copilot-panel-header">
        <div class="dropdown" style="display: flex; align-items: center; gap: 10px;">
            <select id="ai-copilot-menu" style="padding: 4px 8px; border-radius: 4px;">
                <option value="add_module">إضافة مديول</option>
                <option value="edit_module">تعديل مديول</option>
                <option value="ask">إسأل</option>
            </select>
            <span style="font-weight: bold;">AI Alphasky Copilot</span>
        </div>
