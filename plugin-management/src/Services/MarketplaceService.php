...
use Alphasky\Base\Exceptions\RequiresLicenseActivatedException;
use Alphasky\Base\Supports\Core;
use Alphasky\Base\Supports\Zipper;
use Exception;
...
class MarketplaceService
{
    ...
    public function callApi(string $method, string $path, array $request = []): JsonResponse|Response
    {
        ...
        if ($response->failed()) {
            throw new Exception(
                $response->json('message') ?: trans('packages/plugin-management::marketplace.could_not_connect'),
                $response->status()
            );
        }
